...
`apps/desktop` 代码在 WSL 文件系统里，Windows 通过 `\\wsl$\Ubuntu-22.04\` 访问。Electron 主进程必须在 Windows 上跑，才能验证真实的 Windows 行为（文件锁、进程管理、编码、托盘等）：

```powershell
# Windows PowerShell
cd "\\wsl$\Ubuntu-22.04\home\ke\projects\AIASys\apps\desktop"
...
- **提交前必须执行文档与 skill 同步检查**。改了用户-facing 功能要同步更新 `docs/guides/`；改了 skill 要检查关联引用；改了 AGENTS.md 约束要检查受影响的 skill。具体检查清单见 `github-project-management` skill。禁止文档未同步就 commit（AGENTS.md 本身被 gitignore，其改动不进入版本控制，但 skill 引用检查仍然要做）
